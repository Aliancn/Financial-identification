...
def clean_and_normalize_text(text):
    ...
    full_to_half = {ord(c): ord(c) - 0xFEE0 for c in "１２３４５６７８９０ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚ"}
    full_to_half[ord('　')] = ord(' ')  # 全角空格 -> 普通空格
    ...
    try:
        for i, text in enumerate(texts):
            ...
            if any(keyword in normalized_text for keyword in ["开票人", "开票", "票人", "开人"]):
                ...
                # 优先在当前字段中直接提取人名
                match = re.search(r"(开票人|开票|票人|开人)[：:]?\s*(\S+)", normalized_text)
                if match:
                ...
                extended_y_range = (y_center - field_height, y_center + field_height)
                extended_x_range = (x_center - 1.5 * field_width, x_center + 1.5 * field_width)
                ...
                if i + 1 < len(texts):
                    ...
                    next_y_coords = [dt_polys[i + 1][0][1], dt_polys[i + 1][2][1]]
        ...
        try:
            ...
            for i, text in enumerate(texts):
                ...
                if "价税合计" in normalized_text:
                    y_coords = [dt_polys[i][0][1], dt_polys[i][2][1]]  # 获取纵坐标范围 [y1, y3]
            ...
            if price_y_coord and price_height:
                ...
                for i, text in enumerate(texts):
                    ...
                    if extended_y_range[0] <= y_center <= extended_y_range[1]:
                        ...
                        if match:
                            total_price = match.group(1).strip().replace("¥", "").replace("￥", "")
            ...
            if not total_price:
                for i, text in enumerate(texts):
                    ...
                    if "小写" in normalized_text:
                        # 检查当前字段
                        match = re.search(r"([¥￥]?\s*[0-9]+(\.[0-9]{1,2})?)", normalized_text)
                        if match:
                            total_price = match.group(1).strip().replace("¥", "").replace("￥", "")
                        ...
                        if i + 1 < len(texts):
                            ...
                            match = re.search(r"([¥￥]?\s*[0-9]+(\.[0-9]{1,2})?)", next_text)
                            if match:
                                total_price = match.group(1).strip().replace("¥", "").replace("￥", "")
...
def extract_purchaser_register_num(texts, dt_polys, page_width, page_height):
    ...
    try:
        ...
        for i, text in enumerate(texts):
            ...
            normalized_text = text.replace(":", "：").strip()
            num_match = re.search(r"(统一社会信用代码|纳税人识别号)[/／]?(纳税人识别号)?[：:]?\s*(.+)", normalized_text)
            if num_match:
                purchaser_register_num = num_match.group(3).strip()
                cleaned_register_num = purchaser_register_num.replace(" ", "")  # 去掉字段中所有空格
    ...
    try:
        ...
        for i, text in enumerate(texts):
            ...
            if num_match:
                ...
                cleaned_register_num = seller_register_num.replace(" ", "")  # 去掉字段中所有空格
...
def extract_invoice_fields(json_data):
    # 初始化字段字典
    fields = {
        "InvoiceNum": "",  # 发票号
        "InvoiceDate": "",  # 发票日期
    ...
    fields["TotalAmount"] =extract_total_amount_without_tax(texts,dt_polys,page_height)
    fields["TotalTax"] = extract_total_tax(texts,dt_polys,page_width,page_height)
    fields["SellerName"] = extract_seller_name(texts,dt_polys,page_width,page_height)
    fields["SellerRegisterNum"] =extract_seller_register_num(texts,dt_polys,page_width,page_height)
    fields["PurchaserName"] = extract_purchaser_name(texts,dt_polys,page_width,page_height)
    fields["PurchaserRegisterNum"] = extract_purchaser_register_num(texts,dt_polys,page_width,page_height)
    fields["totalPay"]=extract_total_price(texts, dt_polys)
    fields["CommodityDetails"]=extract_project_name(texts,dt_polys,page_height)

    # tolerance = 0.01
    # # 修正 totalPay 的值
    # if abs((fields["TotalAmount"] + fields["TotalTax"]) - fields["totalPay"]) > tolerance:
    #     corrected_total_pay = fields["TotalAmount"] + fields["TotalTax"]  # 计算修正后的总金额
    #     fields["totalPay"] = corrected_total_pay  # 更新字典中的 total_pay
...
def auto_correct_total_pay(data, tolerance=0.01):
    total_amount = round(data.get("TotalAmount", 0), 2)
    total_tax = round(data.get("TotalTax", 0), 2)
    total_pay = round(data.get("totalPay", 0), 2)

    # 检查是否需要修正
    if abs((total_amount + total_tax) - total_pay) > tolerance:
        total_pay = total_amount + total_tax  # 修正 totalPay
        data["totalPay"] = total_pay

    return data

def find_with_json(json_path):

    # json_path = "./output/9.json"
    ...
    # 提取字段
    extracted_fields = extract_invoice_fields(json_data)

    # 输出提取结果
    # print(json.dumps(extracted_fields, ensure_ascii=False, indent=4))
